...
def _relative_path_is_ignored(rel: str, matcher: DimerIgnoreMatcher) -> bool:
    if matcher.is_ignored(rel):
        return True

    # Dimer's own artifact outputs remain visible unless the user explicitly
    # ignores a narrower artifact path above.
    if rel == ARTIFACTS_PREFIX.rstrip("/") or rel.startswith(ARTIFACTS_PREFIX):
        return False

    return any(part in BUILTIN_IGNORE_PARTS for part in Path(rel).parts)


def is_path_ignored_lexically(
    path: Path,
    workspace: Path,
    matcher: DimerIgnoreMatcher | None = None,
) -> bool:
    """Match an in-workspace path before resolving symlinks."""
    ...
    try:
        rel = str(lexical.relative_to(ws)).replace("\\", "/")
    except ValueError:
        return False
    return _relative_path_is_ignored(
        rel,
        matcher or DimerIgnoreMatcher(workspace=ws),
    )
