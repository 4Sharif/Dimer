...
def _profile_column(series: pd.Series) -> ColumnProfile:
    ...
    # Keep structural diagnostics, but never retain values or derived ranges
    # for fields whose names indicate credentials.
    if is_sensitive_field_name(str(series.name)):
        return profile
    ...
    if pd.api.types.is_numeric_dtype(series):
        ...
        profile.categorical_top_values = [
            {
                "value": redact_sensitive_text(str(idx)),
                "count": int(cnt),
            }
...
def profile_dataset(
    ...
    # Prefer richer diagnostics; keep legacy warnings as fallback seeds.
    from dimer.data_context.data_quality import quality_messages

    richer = quality_messages(profile, df)
    profile.quality_warnings = list(dict.fromkeys([*richer, *warnings]))
    return profile
...
def save_profile(profile: DatasetProfile, workspace: Path | None = None) -> Path:
    ...
    safe_profile = redact_sensitive_data(profile.model_dump(mode="json"))
    out.write_text(json.dumps(safe_profile, indent=2), encoding="utf-8")
    return out
