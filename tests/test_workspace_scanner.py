...
def test_dimer_artifacts_are_visible_without_becoming_source_files(tmp_path: Path) -> None:
    artifact = tmp_path / ".dimer" / "artifacts" / "queries" / "evidence.sql"
    artifact.parent.mkdir(parents=True)
    artifact.write_text("SELECT 1", encoding="utf-8")
    internal = tmp_path / ".dimer" / "analysis_state.jsonl"
    internal.write_text("{}\n", encoding="utf-8")

    scan = scan_workspace(tmp_path)

    assert scan["artifacts"] == [".dimer/artifacts/queries/evidence.sql"]
    assert scan["sql_files"] == []
